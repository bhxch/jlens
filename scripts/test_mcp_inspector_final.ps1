...
function Test-Tool {
    ...
    try {
        # Build command
        $cmdParts = @("npx", "@modelcontextprotocol/inspector", "--cli", "--config", $ConfigPath, "--server", $ServerName, "--method", "tools/call", "--tool-name", $ToolName)
        if ($Arguments) {
            $cmdParts += $Arguments.Split(' ')
        }
        $cmd = $cmdParts -join ' '
        
        $output = & $cmdParts 2>&1 | Out-String
...
try {
    $cmdParts = @("npx", "@modelcontextprotocol/inspector", "--cli", "--config", $ConfigPath, "--server", $ServerName, "--method", "tools/list")
    $output = & $cmdParts 2>&1 | Out-String
