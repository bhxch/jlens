# MCP Inspector Integration Test Script
...
function Test-Tool {
    ...
    $TotalTests++
    Write-Host "[$TotalTests] Testing: $TestName..." -ForegroundColor Yellow
    
    try {
        $cmd = "npx @modelcontextprotocol/inspector --cli --config `"$ConfigPath`" --server $ServerName --method tools/call --tool-name $ToolName $Arguments 2>&1"
...
$TotalTests++
Write-Host "[$TotalTests] Testing: List tools..." -ForegroundColor Yellow
try {
    $cmd = "npx @modelcontextprotocol/inspector --cli --config `"$ConfigPath`" --server $ServerName --method tools/list 2>&1"
    $output = Invoke-Expression $cmd | Out-String
...
Write-Host "Pass Rate: $([math]::Round(($PassedTests / $TotalTests) * 100, 2))%" -ForegroundColor $(if ($PassedTests -eq $TotalTests) { "Green" } else { "Yellow" })
Write-Host ""
