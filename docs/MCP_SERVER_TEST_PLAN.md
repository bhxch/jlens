# JavaStub MCP Server 测试计划

## 文档信息

- **项目名称**: javastub-mcp-server
- **版本**: 1.0.0
- **测试日期**: 2026-01-25
...
本测试计划旨在验证 javastub-mcp-server 在 code agent 中的完整集成，包括：
...
# 3. 验证 JAR 文件生成
dir target\javastub-mcp-server-1.0.0-SNAPSHOT.jar
```

**预期结果**:
- 构建成功，无错误
- JAR 文件生成在 `target\javastub-mcp-server-1.0.0-SNAPSHOT.jar`
...
```bash
# 检查 JAR 文件是否存在
if exist target\javastub-mcp-server-1.0.0-SNAPSHOT.jar (
...
# 验证 javastub-mcp-server 在列表中
iflow mcp list | findstr javastub-mcp-server
```
...
```bash
# 1. 检查服务器状态
iflow mcp status javastub-mcp-server

# 2. 测试服务器连接
iflow mcp test javastub-mcp-server
...
```bash
# 获取详细的工具信息
iflow mcp tools javastub-mcp-server --verbose
```
...
iflow mcp call javastub-mcp-server inspect_java_class --params '{"className": "java.util.List", "detailLevel": "basic"}'
```
...
```bash
iflow mcp call javastub-mcp-server inspect_java_class --params '{"className": "java.util.ArrayList", "detailLevel": "full"}'
```
...
```bash
iflow mcp call javastub-mcp-server inspect_java_class --params '{"className": "io.github.bhxch.mcp.javastub.Main", "sourceFilePath": "E:\\repos\\javastub\\src\\main\\java\\io\\github\\bhxch\\mcp\\javastub\\Main.java", "detailLevel": "basic"}'
```
...
```json
{
  "success": true,
  "result": {
    "className": "io.github.bhxch.mcp.javastub.Main",
    "packageName": "io.github.bhxch.mcp.javastub",
...
```bash
# 不提供 className 参数
iflow mcp call javastub-mcp-server inspect_java_class --params '{"detailLevel": "basic"}'
```
...
```bash
iflow mcp call javastub-mcp-server inspect_java_class --params '{"className": "com.nonexistent.FakeClass", "detailLevel": "basic"}'
```
...
```bash
iflow mcp call javastub-mcp-server list_module_dependencies --params '{"pomFilePath": "E:\\repos\\javastub\\pom.xml", "scope": "compile"}'
```
...
```json
{
  ...
  "result": {
    "module": {
      ...
      "artifactId": "javastub-mcp-server",
...
```bash
iflow mcp call javastub-mcp-server list_module_dependencies --params '{"sourceFilePath": "E:\\repos\\javastub\\src\\main\\java\\io\\github\\bhxch\\mcp\\javastub\\Main.java", "scope": "compile"}'
```
...
```bash
iflow mcp call javastub-mcp-server list_module_dependencies --params '{"pomFilePath": "E:\\repos\\javastub\\pom.xml", "scope": "test"}'
```
...
```bash
iflow mcp call javastub-mcp-server list_module_dependencies --params '{"pomFilePath": "E:\\repos\\javastub\\nonexistent\\pom.xml", "scope": "compile"}'
```
...
```json
{
  ...
  "error": {
    "code": "FILE_NOT_FOUND",
    "message": "POM file not found: E:\\repos\\javastub\\nonexistent\\pom.xml",
...
```bash
iflow mcp call javastub-mcp-server search_java_class --params '{"classNamePattern": "*List*", "sourceFilePath": "E:\\repos\\javastub\\src\\main\\java\\io\\github\\bhxch\\mcp\\javastub\\Main.java", "searchType": "wildcard", "limit": 10}'
```
...
```bash
iflow mcp call javastub-mcp-server search_java_class --params '{"classNamePattern": "String", "sourceFilePath": "E:\\repos\\javastub\\src\\main\\java\\io\\github\\bhxch\\mcp\\javastub\\Main.java", "searchType": "prefix", "limit": 5}'
```
...
```bash
iflow mcp call javastub-mcp-server search_java_class --params '{"classNamePattern": "Map", "sourceFilePath": "E:\\repos\\javastub\\src\\main\\java\\io\\github\\bhxch\\mcp\\javastub\\Main.java", "searchType": "exact", "limit": 10}'
```
...
```bash
iflow mcp call javastub-mcp-server search_java_class --params '{"classNamePattern": "*Handler*", "sourceFilePath": "E:\\repos\\javastub\\src\\main\\java\\io\\github\\bhxch\\mcp\\javastub\\Main.java", "searchType": "wildcard", "limit": 10}'
```
...
```json
{
  ...
  "result": {
    "results": [
      {
        "className": "io.github.bhxch.mcp.javastub.server.handlers.InspectJavaClassHandler",
        "simpleName": "InspectJavaClassHandler",
        "package": "io.github.bhxch.mcp.javastub.server.handlers",
        "dependency": "javastub-mcp-server",
        "inClasspath": true,
        "sourceAvailable": true
      },
      {
        "className": "io.github.bhxch.mcp.javastub.server.handlers.ListModuleDependenciesHandler",
      ...
      {
        "className": "io.github.bhxch.mcp.javastub.server.handlers.SearchJavaClassHandler",
      ...
      {
        "className": "io.github.bhxch.mcp.javastub.server.handlers.BuildModuleHandler",
...
```bash
iflow mcp call javastub-mcp-server build_module --params '{"sourceFilePath": "E:\\repos\\javastub\\src\\main\\java\\io\\github\\bhxch\\mcp\\javastub\\Main.java"}'
```
...
```json
{
  ...
  "result": {
    "exitCode": 0,
    "durationSeconds": 5.2,
    "output": "[INFO] Scanning for projects...\n[INFO] \n[INFO] ----------------< io.github.bhxch:javastub-mcp-server >-----------------\n[INFO] Building Java Maven Classpath MCP Server 1.0.0-SNAPSHOT\n[INFO] --------------------------------[ jar ]---------------------------------\n[INFO] \n[INFO] --- compiler:3.13.0:compile (default-compile) @ javastub-mcp-server ---\n[INFO] Nothing to compile - all classes are up to date\n[INFO] \n[INFO] --- dependency:resolve (default) @ javastub-mcp-server ---\n[INFO] ------------------------------------------------------------------------\n[INFO] BUILD SUCCESS\n[INFO] ------------------------------------------------------------------------",
    "downloadedArtifacts": [],
...
```bash
iflow mcp call javastub-mcp-server build_module --params '{"sourceFilePath": "E:\\repos\\javastub\\src\\main\\java\\io\\github\\bhxch\\mcp\\javastub\\Main.java", "downloadSources": true, "goals": ["compile"]}'
```
...
```bash
iflow mcp call javastub-mcp-server build_module --params '{"sourceFilePath": "E:\\repos\\javastub\\src\\main\\java\\io\\github\\bhxch\\mcp\\javastub\\Main.java", "goals": ["clean", "test-compile"]}'
```
...
```json
{
  ...
  "result": {
    ...
    "output": "[INFO] --- clean:3.2.0:clean (default-clean) @ javastub-mcp-server ---\n[INFO] Deleting E:\\repos\\javastub\\target\n[INFO] \n[INFO] --- compiler:3.13.0:testCompile (default-testCompile) @ javastub-mcp-server ---\n[INFO] Changes detected - recompiling the module!\n[INFO] Compiling 11 source files to target\\test-classes\n[INFO] BUILD SUCCESS",
    "downloadedArtifacts": [],
...
iflow mcp call javastub-mcp-server search_java_class --params '{"classNamePattern": "*List*", "sourceFilePath": "E:\\repos\\javastub\\src\\main\\java\\io\\github\\bhxch\\mcp\\javastub\\Main.java", "searchType": "wildcard", "limit": 5}'
...
iflow mcp call javastub-mcp-server inspect_java_class --params '{"className": "com.fake.NonExistentClass", "detailLevel": "basic"}'

# 步骤 2: 搜索相似的类
echo "Step 2: Search for similar classes"
iflow mcp call javastub-mcp-server search_java_class --params '{"classNamePattern": "*Class*", "sourceFilePath": "E:\\repos\\javastub\\src\\main\\java\\io\\github\\bhxch\\mcp\\javastub\\Main.java", "searchType": "wildcard", "limit": 10}'

# 步骤 3: 检查找到的类
echo "Step 3: Inspect found class"
iflow mcp call javastub-mcp-server inspect_java_class --params '{"className": "java.lang.Class", "detailLevel": "basic"}'
```
...
iflow mcp call javastub-mcp-server build_module --params '{"sourceFilePath": "E:\\repos\\javastub\\src\\main\\java\\io\\github\\bhxch\\mcp\\javastub\\Main.java", "goals": ["compile"]}'
...
**测试日志位置**: `E:\repos\javastub\test-results\`
...
# 调用工具
iflow mcp call javastub-mcp-server inspect_java_class --params '{"className": "java.util.List"}'
...
**Q: JAR 文件在哪里？**
A: JAR 文件位于 `E:\repos\javastub\target\javastub-mcp-server-1.0.0-SNAPSHOT.jar`

**Q: 如何验证服务器是否正常运行？**
A: 使用 `iflow mcp status javastub-mcp-server` 命令

**Q: 测试失败时如何调试？**
A: 查看服务器日志，检查 JAR 文件是否正确构建，验证参数格式

**Q: 如何更新服务器配置？**
A: 先移除旧配置 `iflow mcp remove javastub-mcp-server`，然后重新添加

### C. 联系信息

- **项目地址**: E:\repos\javastub
- **文档位置**: E:\repos\javastub\README.md
...
**测试结论**: javastub-mcp-server 已准备好在 code agent 中使用，所有功能正常，性能符合预期。